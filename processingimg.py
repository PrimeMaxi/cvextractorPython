...
def draw_blocks(img, blocks, zoom1,zoom2):
  for i in range(len(blocks)):
    print()
    (left_i, top_i, width_i, height_i) = (blocks[i]['x']*zoom1, blocks[i]['y']*zoom2, blocks[i]['width']*zoom1, blocks[i]['height']*zoom2)
...
def search_words(img):
  ...
  for i in range(n_boxes): #cicla su tutti i blocchi di testo present

      if(text['level'][i] == 5) :
        (left_i, top_i, width_i, height_i) = (text['left'][i]-3, text['top'][i]-3, text['width'][i]+6, text['height'][i]+6)
        
        if (height_i <= 6):#controllo (non posso fa di più se no si prende i caratteri tipo :) --> si può anche cancellare a sto punto
          continue
        ...
        text_block=extract_text(img,left_i,top_i,width_i,height_i,7) #dovresti provare a leggere già dal pdf 
...
##rimuovere blocchi molto lunghi o molto larghi o che sono quadrati grandi 
#rimuovere caratteri speciali e vedere se la stringa è vuota
#rimuovere blocchi grandi di una certa dimensione e che togliendo i caratteri speciali contengono poco testo
