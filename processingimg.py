...
def search_words(img):
    ...
    for i in range(n_boxes):  # cicla su tutti i blocchi di testo present
        ...
        if (text['level'][i] == 5):
            ...
            if (
                    height_i <= 6):  # controllo (non posso fa di più se no si prende i caratteri tipo :) --> si può anche cancellare a sto punto
                continue
            ...
            text_block = extract_text(img, left_i, top_i, width_i, height_i,
                                      7)  # dovresti provare a leggere già dal pdf
