  ...
  while finish==1:
    ...
    for i in range(n_words):
      ...
      for j in range(n_words):
        (left_j, top_j, width_j, height_j) = (words[j]['x'], words[j]['y'], words[j]['width'], words[j]['height'])
        

        if (i!=j):
#meglio il min perchè se sono stessa riga di testo molto probabilmente hanno anche una simile altezzza --> sfavorisce blocchi vicini con altezze abbastanza diverse
          if (left_j + width_j >= left_i + width_i and left_j - min(height_i,height_j)*0.95  <=  left_i+width_i and top_j + height_j <= top_i+height_i+6 and top_j >= top_i - 6): #and left_iplus - height_i <=  left_i+width_i    

            separate = separate_rect(left_i, top_i, width_i, height_i, left_j, top_j, width_j, height_j,rects,words[i]['text'],words[j]['text'])
            if (separate==True):
              continue
...
def separate_rect(left_i, top_i, width_i, height_i, left_j, top_j, width_j, height_j, rects,text1,text2):
  for rect in rects:
    if (left_i+width_i<=rect['x'] and left_j + 3 >=rect['x']+rect['width'] and \
       ( ( rect['y']+rect['height']>min(top_i,top_j) and rect['y']+rect['height']<max(top_i+height_i,top_j+height_j) ) or \
         ( rect['y']>=min(top_i,top_j) and rect['y']<=max(top_i+height_i,top_j+height_j)  )  or\
         (rect['y']<=min(top_i,top_j) and rect['y']+rect['height']>=max(top_i+height_i,top_j+height_j)) ) ):
...
def search_blocks_ocr(words):
  ...
  while finish==1:
    ...
    for i in range(n_words):
      (left_i, top_i, width_i, height_i) = (words[i]['left'], words[i]['top'], words[i]['width'], words[i]['height'])
      for j in range(n_words):
        (left_j, top_j, width_j, height_j) = (words[j]['left'], words[j]['top'], words[j]['width'], words[j]['height'])

        if (i!=j  and left_j + width_j >= left_i + width_i and left_j - round(max(height_i,height_j)*1.1,0) <=  left_i+width_i and top_j + height_j <= top_i+height_i+6 and top_j >= top_i - 6): #and left_iplus - height_i <=  left_i+width_i     
...
def union_blocks(blocks): #blocchi contenuti in altri blocchi
  ...
  while finish==1:
    ...
    for i in range(numblocks): #cicla su tutti i blocchi di testo presenti top_iplus >= top_i-6 ... top plus + height plus  <= top + height +6 
      (left_i, top_i, width_i, height_i) = (blocks[i]['x'], blocks[i]['y'], blocks[i]['width'], blocks[i]['height'])
      for j in range(numblocks):
        (left_j, top_j, width_j, height_j) = (blocks[j]['x'], blocks[j]['y'], blocks[j]['width'], blocks[j]['height'])
        if (i!=j):
          #va a vedere tutti i casi di blocchi contenuti in altri (in tutte le posizioni possibili)... sicuramente c'è un modo per compattare le varie operazioni logiche CONTROLLARE
          if ((left_j+width_j <= left_i + width_i and left_j <= left_i and left_i <= left_j+width_j and top_j <= top_i and top_i <= top_j + height_j and top_j+height_j <= top_i + height_i) or \
